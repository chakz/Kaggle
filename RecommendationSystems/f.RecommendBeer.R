f.RecommendBeer <- function(path.to.data.csv, myKindOfBeer, myStyleOfBeer, numberOfRecommendations){
  # Recommendation system for suggesting beers #
  # arguments:
  #         path.to.data.csv: The path to the beer review csv file
  # Returns:
  #           A list for the 5 similar or recommended types of beers based on your preference
  
  # reading in the data to beerData
  beerData <- read.csv(path.to.data.csv)
  
  # Organizing the beerData file
  ...
  # Filtering the beers based on their id, and selecting the beers which have received more than
  # 500 reviews
  
  beerData_numReviews <- data.frame(table(beerData$beer_beerid))
  
  # Renaming the tabular data columns
  names(beerData_numReviews)[1] <- "beer_beerid"
  
  # Selecting the beers which have got more than 500 reviews
  ...
  find_similar_beers <- function(mybeer, style, numberOfRecommendations ){
    ...
    numberOfRecommendations <- min(numberOfRecommendations, nrow(similar))
    similar <- similar[1:numberOfRecommendations, c("brewery_name", "beer_name", "beer_style", "sim")]
    similar
  }
  myBeer <- myKindOfBeer
  myStyle <- myStyleOfBeer
  nRecos <- numberOfRecommendations
  
  result <- find_similar_beers(myBeer, myStyle, nRecos)
